...
import { addDoc, arrayUnion, collection, doc, getDoc, getDocs, updateDoc, where } from "firebase/firestore";
import { delay, put, select, takeLatest } from "redux-saga/effects";
import { auth, fs } from "../../services/firebase";
import { addChatToStore, setSubscribeOnNewChats } from "../actions";
import { ADD_CHAT_WITH_SAGA, GET_CHATS_WITH_SAGA } from "../types";
...
function* addDialogWorker( uidAnotherUser ) {
    try {
        ...
        const refDialog = yield addDoc( collection( fs, 'chats' ), {
            messages: [],
        } );
        // update currentUser add new dialog
        yield updateDoc( doc( fs, 'users', auth.currentUser.uid ), {
            dialogs: arrayUnion( { chatId: refDialog.id, emailAnotherUser: user.email, uidAnotherUser: user.uid, anotherName: user.displayName } )
        }, )
        // update anotherUser, add new dialog
        yield updateDoc( doc( fs, 'users', user.uid ), {
            dialogs: arrayUnion( { chatId: refDialog.id, emailAnotherUser: auth.currentUser.email, uidAnotherUser: auth.currentUser.uid, anotherName: auth.currentUser.displayName } )
        }, )
    } catch ( error ) {
